...
impl HashesContainer {
    pub fn new(first_index: usize) -> Self {
        Self {
            working_tree: IndexMap::with_chunk_capacity_on_disk(100_000, 20, 100_000),
            commiting: IndexMap::with_chunk_capacity_on_disk(100_000, 20, 100_000),
            is_commiting: false,
            first_index,
        }
    }
    ...
    pub fn commited(&mut self) {
        ...
        if self.working_tree.capacity() > 1000 {
            self.working_tree = IndexMap::with_chunk_capacity_on_disk(100_000, 20, 100_000);
        } else {
            self.working_tree.clear();
        }
        if self.commiting.capacity() > 1000 {
            self.commiting = IndexMap::with_chunk_capacity_on_disk(100_000, 20, 100_000);
        } else {
