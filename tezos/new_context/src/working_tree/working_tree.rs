...
use std::{array::TryFromSliceError, borrow::Cow, cell::{Cell, RefCell}, rc::Rc, sync::{Arc, PoisonError}};

use failure::Fail;

use crypto::hash::FromBytesError;

use crate::{ContextKeyValueStore, persistent::self};
...
impl WorkingTree {
    ...
    */
    ...
    fn get_key_values_from_tree_recursively(
        ...
        match entry {
                ...
                tree.iter()
                    .map(|(key, child_node)| {
                        ...
                        match self.node_entry(&child_node) {
                            Err(_) => Ok(()),
                            Ok(entry) => self
                                .get_key_values_from_tree_recursively(&fullpath, &entry, entries, store),
                        }
                    })
            ...
            Entry::Commit(commit) => match self.get_entry_from_hash_id(commit.root_hash, store) {
                Err(err) => Err(err),
                Ok(entry) => self.get_key_values_from_tree_recursively(path, &entry, entries, store),
            },
        }
    }
    ...
    pub fn get_working_tree_root_hash(&self, store: &mut ContextKeyValueStore) -> Result<HashId, MerkleError> {
    ...
    fn get_entry_from_hash_id(&self, hash_id: HashId, store: &ContextKeyValueStore) -> Result<Entry, MerkleError> {
