...
use crate::{EntryHash, gc::{NotGarbageCollected, repository::{HashId, VacantEntryHash}}, persistent::KeyValueStoreBackend};
...
impl IpcContextClient {
    ...
    pub fn get_value(
        &self,
        hash_id: HashId,
    ) -> Result<Option<Cow<[u8]>>, ContextServiceError> {
        ...
        {
            ContextResponse::GetValueResponse(result) => {
                result.map(|h| h.map(|h| Cow::Owned(h))).map_err(|err| ContextError::GetValueError { reason: err }.into())
            }
    ...
    /// Check if entry with hash id exists
    pub fn get_context_hash_id(&self, context_hash: &ContextHash) -> Result<Option<HashId>, ContextServiceError> {
        let mut io = self.io.borrow_mut();
        io.tx.send(&ContextRequest::GetContextHashId(context_hash.clone()))?;
    ...
    pub fn get_hash(&self, hash_id: HashId) -> Result<Option<Cow<EntryHash>>, ContextServiceError> {
        ...
        match io
            ...
            .try_receive(Some(Self::TIMEOUT), Some(IpcContextListener::IO_TIMEOUT))?
        {
            ContextResponse::GetContextHashResponse(result) => {
                result.map(|h| h.map(|h| Cow::Owned(h))).map_err(|err| ContextError::GetContextHashError { reason: err }.into())
            }
            message => Err(ContextServiceError::UnexpectedMessage {
                message: message.into(),
            }),
                ...
                ContextRequest::GetContextHashId(context_hash) => {
                    match crate::ffi::get_context_index() {
                        ...
                        Some(index) => {
                            ...
                            io.tx.send(&ContextResponse::GetContextHashIdResponse(res))?;
                        }
                    }
                }
                ContextRequest::GetHash(hash_id) => {
                    match crate::ffi::get_context_index() {
