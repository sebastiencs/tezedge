...
use std::{borrow::Borrow, cell::RefCell, collections::HashSet, convert::TryInto, sync::{Arc, RwLock}};
use std::rc::Rc;

use crypto::hash::ContextHash;
use ocaml_interop::BoxRoot;

use crate::{ContextKeyValueStore, StringTreeMap, gc::repository::HashId, hash::EntryHash, persistent::DBError, working_tree::{Commit, Entry, KeyFragment, Node, Tree, working_tree::MerkleError, working_tree_stats::MerkleStoragePerfReport}};
...
impl TezedgeIndex {
    ...
    pub fn get_hash(&self, hash_id: HashId) -> Result<Option<EntryHash>, DBError> {
        Ok(self.repository.read()?.get_hash(hash_id)?.map(|h| h.into_owned()))
    }
    ...
    pub fn get_context_hash_id(&self, context_hash: &ContextHash) -> Result<Option<HashId>, MerkleError> {
        let db = self.repository.read()?;
...
impl ShellContextApi for TezedgeContext {
    fn commit(
        ...
        let (commit_hash_id, batch, referenced_older_entries) =
            self.tree
                .prepare_commit(date, author, message, self.parent_commit_hash, &mut *repository)?;
