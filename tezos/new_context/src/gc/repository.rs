...
impl HashValueStore {
    ...
    pub(crate) fn put_hash(&mut self, hash: EntryHash) -> HashId {
        let hash_id = if let Some(free_id) = self.free_ids.pop().ok() {
            self.values[free_id] = None;
            self.hashes[free_id] = hash;
            free_id
        } else {
    ...
    pub(crate) fn push_hash_with<F>(&mut self, fun: F) -> HashId
    where
        F: FnOnce(&mut EntryHash),
    {
        let hash_id = if let Some(free_id) = self.free_ids.pop().ok() {
            self.values[free_id] = None;
            fun(&mut self.hashes[free_id]);
            free_id
        } else {
            self.hashes.push_with(fun)
        };
        self.new_ids.push(hash_id);
        hash_id
    }
...
impl Repository {
    ...
    pub fn get_context_hash(&self, context_hash: &ContextHash) -> Option<HashId> {
    ...
    pub fn put_context_hash(&mut self, commit_hash_id: HashId) -> ContextHash {
