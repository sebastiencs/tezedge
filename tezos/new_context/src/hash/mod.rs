    ...
    if entries.is_empty() {
        ...
        for i in 0..=31 {
            ...
            match ti {
                Inode::Empty => (),
                non_empty => {
                    pointers.push((i as u8, hash_long_inode(&non_empty, store, tree_storage)?))
                }
            }
        }
    ...
    match inode {
        ...
        Inode::Value(entries) => {
            ...
            for (name, node_id) in entries {
                ...
                hasher.update(node.entry_hash(store, tree_storage)?.as_ref());
            }
        }
        Inode::Tree {
            depth,
...
fn hash_short_inode(
    ...
    for (k, v) in tree {
        ...
        hasher.update(v.entry_hash(store, tree_storage)?.as_ref());
    }
