...
use crate::{ContextKeyValueStore, gc::repository::HashId, persistent::{BincodeEncoded, DBError}, working_tree::{Commit, Entry, Node, NodeKind, Tree}};
...
fn hash_long_inode(inode: &Inode, store: &mut ContextKeyValueStore) -> Result<HashId, HashingError> {
...
pub(crate) fn hash_tree(tree: &Tree, store: &mut ContextKeyValueStore) -> Result<HashId, HashingError> {
...
mod tests {
    use std::{cell::{Cell, RefCell}, convert::TryInto, env, fs::File, io::Read, path::Path, rc::Rc};

    use flate2::read::GzDecoder;

    use crypto::hash::{ContextHash, HashTrait};

    use crate::{gc::repository::Repository, working_tree::{Node, NodeKind, Tree}};
    ...
    fn test_hash_of_commit() {
        ...
        let vacant = repo.get_vacant_entry_hash();
        let hash_id = vacant.write_with(|entry| {
            *entry = hex::decode(
    ...
    fn test_type_hashes(json_gz_file_name: &str) {
        ...
        for test_case in test_cases {
            ...
            for binding in test_case.bindings {
                ...
                let vacant = repo.get_vacant_entry_hash();
                let hash_id = vacant.write_with(|entry| {
                    *entry = entry_hash.as_ref().as_slice().try_into().unwrap()
                });
